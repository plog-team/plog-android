...
public class BookmarkFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        // 상태바 높이만큼 상단 여백 적용
        View statusBarSpace = view.findViewById(R.id.statusBarSpace);
        statusBarSpace.getViewTreeObserver().addOnGlobalLayoutListener(() -> {
            int statusBarHeight = 0;
            int resourceId = requireContext().getResources()
                    .getIdentifier("status_bar_height", "dimen", "android");
            if (resourceId > 0) {
                statusBarHeight = requireContext().getResources().getDimensionPixelSize(resourceId);
            }
